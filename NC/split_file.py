...
Dst_dir="D:\\tmp\\R4scripts\\2\\"
# проверяем наличие директории с данным месяцем
if not os.path.exists(Dst_dir): os.mkdir(Dst_dir)


s_file=open(src)
l_file=open(Dst_dir+'all_round_scripts.txt','w')
...
for text in s_list:
   ...
   if (text.find('914')==0):
       ...
       if i>0 :
          ...
          w_file = open(Dst_dir+o_id +'_'+name+'.txt','w')
          print ('File: '+Dst_dir+o_id +'_'+name+' is opened:'+format(not(w_file.closed)))
          w_file.write(script)
          w_file.close()
          print ('File: '+Dst_dir+o_id +'_'+name+' is closed: '+ format(w_file.closed))
       i=i+1
       spl=text.split(";")
       #print i
       o_id=spl[0]
